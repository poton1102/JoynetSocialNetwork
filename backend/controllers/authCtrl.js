...
const authCtrl = {
    register: async (req, res) => {
        try {
            ...
            if (user_email) return res.status(400).json({ msg: "Địa chỉ email đã tồn tại." })

            if (password.length < 6)
                return res.status(400).json({ msg: "Mật khẩu phải có tối thiểu 6 ký tự." })
    ...
    generateAccessToken: async (req, res) => {
        try {
            ...
            if (!rf_token) return res.status(400).json({ msg: "Hãy đăng nhập ngay." })

            jwt.verify(rf_token, process.env.REFRESH_TOKEN_SECRET, async (err, result) => {
                if (err) return res.status(400).json({ msg: "Hãy đăng nhập ngay." })
